...
Parse the topic or decision, seed paths, and scope constraints such as `docs/ only`, `skip research/`, or `implementation docs only`. If the topic is genuinely ambiguous, use AskUserQuestion for one pointed question; otherwise proceed with the supplied scope.
