---
name: goal-prompt
description: Create a compact durable goal handoff for a Claude Code session. Use when the user invokes goal-prompt or asks for a resumable, autonomous, research, or PRD-backed Claude handoff.
---

# Goal Prompt

Create one short direct Claude Code prompt backed by one durable goal spec.

The durable goal spec carries the real instructions and survives compaction. The generated prompt only points the target session to that spec and tells it to execute.
...
- This skill targets Claude Code directly.
- Do not tell Claude to set a goal, use `/goal`, create a meta-prompt, or call Codex goal tooling.
...
7. Add a concise `Recommended Runtime` only when delegation cost, ambiguity, risk, or required thoroughness makes the recommendation useful. Keep it advisory unless the user explicitly requires a setting. Follow `docs/model-and-effort.md` when it applies.
...
- **Reviewed behavior spine:** when the task covers a stable, meaningful behavior that spans states or artifacts, depends on a required non-event, or is likely to receive a substantial agent implementation, carry the approved behavior authority, supported public seam, observable consequences and non-events, controlled boundaries, prohibited shortcuts, independent oracle, and claim ceiling into the Completion Contract. Require one plausible production defect to turn the spine red, one semantics-preserving implementation refactor to remain green, and a detector ledger for deleted or weakened tests. Do not add this clause for small local changes or unstable behavior where focused tests are sufficient.
