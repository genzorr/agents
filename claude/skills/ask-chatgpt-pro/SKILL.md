...
description: Prepare a mode-aware, evidence-grounded manual ChatGPT Pro consult document that uses ChatGPT's GitHub connector for repository inspection, verification, synthesis refinement, decision support, or execution planning. Use when Pro should inspect one or more GitHub repos, commits, branches, PRs, or named files directly; especially when local changes must be committed and pushed before Pro can see them.
---
