...
For harness-only checkpointing (with the full lifecycle ceremony) use `/harness-task-checkpoint`; for closing a done task use `/harness-task-done`. This skill is the portable variant.

## Gather

1. Check whether this is a harness repo:
   - Look for `docs/harness/.harness` in the current directory or parents.
   - If present, run `harness snapshot` via Bash once. If it fails, fall back to the non-harness path and mention the failure.
...
4. Read only files needed to name changed artifacts, verification, or unresolved blockers. Prefer Glob / Grep `files_with_matches`; do not do a broad source walk.
