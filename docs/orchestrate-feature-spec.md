...
- Harness-owned records through Harness CLI operations
...
- A general worker override changes the named field for all actual workstreams unless a matching named-workstream override supersedes that field.
- A named-workstream override changes only its explicit model and/or effort fields for matching assignments. Resolve each field in the order role default, general worker override, named-workstream override.
- `Main`, `additional`, and similar workstream names are assignment labels. They do not define new roles, require a worker count, or authorize a launch.
...
- An omitted owner or enabled-reviewer model/effort inherits that role's default and never another role's override; worker fields follow the default/general/named hierarchy above. Reviewer controls remain unresolved while review is disabled.
...
4. One role override cascades to another role, an omitted owner/reviewer field inherits another override, worker resolution skips the default/general/named hierarchy, or the current orchestrator is described as mutable.
...
19. First review covers the complete target; correction-round coverage reuse requires an exact prior baseline and complete delta including uncommitted changes, every mutation invalidates the verdict, and missing coverage or changed consequences expand inspection before a new complete-current-target verdict.
