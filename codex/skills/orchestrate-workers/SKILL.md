...
description: Configure the current task as a coordinator with profiled native implementation workers, an optional directly requested ordinary task, and an independent reviewer only when explicitly operator-requested. Preserve coordinator profile and driver while owning worker/reviewer decomposition, continuity, fresh context, evidence, routing, and result handling. Defaults are native Luna/xhigh workers, coordinator self-review, and requested reviewer native Sol/high. Other profiles require explicit operator instruction or an operator-authorized launch contract. Use only after explicit invocation of orchestrate-workers or a relayed invocation in an operator-authorized feature launch.
---

# Orchestrate Workers

**Composition role: helper.** Keep the current task skill or workflow as driver. It decides whether delegation is allowed and retains lifecycle, success criteria, authority, and acceptance. This helper owns worker/reviewer decomposition, delegation, context, evidence, and explicitly requested independent-review routing; it grants no authority and never changes the current coordinator's profile.

## Uniform Coordinator Invariant

The current task is always the coordinator. This helper has no standalone mode and no feature-owner mode; a feature owner is simply the coordinator when the outer launch assigns that task. Native workers are the default route. An ordinary implementation task requires a direct explicit operator request and existing task-creation authority; a relayed feature launch supplies neither. The coordinator's callback or reporting relationship to an ordinary parent belongs to that outer launch, not to this helper's native worker/reviewer route.
...
- **Coordinator:** current task applying this helper; it may use any product-exposed profile and remains planner, integrator, primary reviewer, and acceptance authority.
...
- **Native subagent route:** worker or reviewer returns only to its immediate parent through native collaboration/results; no ordinary task, sibling, higher-orchestrator, or cross-task route. Parent delivery failure returns natively.
- **Separately authorized ordinary-task route:** only after a direct explicit operator request naming this ordinary implementation-task route and existing task-creation authority. Verify the exact recipient and authorized callback purpose, plus the absence of both `model` and `thinking`; include exact originating coordinator `threadId` and `hostId` when required plus the explicit native `send_message_to_thread` action. Accepted blocker/terminal sends establish delivery; rejection or unavailability remains local delivery failure. Route permission never includes settings changes.
...
Use only on a direct explicit operator request naming this ordinary implementation-task route when the current coordinator already has task-creation authority. A relayed feature launch always supplies neither the direct request nor the authority, so this route is unavailable there. Resolve the exact originating coordinator `threadId` and `hostId` when required from native status and require the explicit native `send_message_to_thread` action; never invent/store a side-channel route. Verify the exact recipient, authorized callback purpose, and absence of both `model` and `thinking` before an ordinary send; route permission never authorizes an existing-task settings change. Use the resolved worker creation profile and the same decomposition, ownership, dependency, reset, and contract rules. Do not create a worktree without approval.

After launch, return the launch response immediately and do not wait or poll. The task sends only a genuine blocker or final handoff to the exact coordinator through native callback, with no `model` or `thinking` fields. An accepted blocker/terminal send establishes delivery; rejection or unavailability remains local delivery failure. This child-task callback does not make the helper the owner of the current coordinator's reporting relationship to an ordinary parent. Reuse only for compatible assignments after reset; across tasks require fresh helper invocation and explicit ordinary-route request. Create a new task only on explicit request or recycle trigger.
