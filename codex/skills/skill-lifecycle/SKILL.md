...
3. Compare responsibilities by description and body, collect cited signals, and choose exactly one lifecycle category per finding. Default thin evidence to `keep + watch`; do not promote one-provider, one-project, or one-implementation evidence into a provider-independent, global, or family-wide rule; require operator approval for merge, deprecate, or remove proposals.
