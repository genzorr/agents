...
When the host provides a native visualization capability, read and follow that capability's full instructions before creating or updating a visual. On Codex, the bundled `visualize` skill is the primary route for this; keep it unchanged and follow its rendering, accessibility, responsive-layout, export, and active-content constraints. Explain owns the narrative, evidence selection, and textual equivalent. Keep the first view useful, keep controls local to the question, and do not invent a custom renderer or copy a host visualizer's implementation contract into this skill.
