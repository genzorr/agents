...
is_repo_managed_skill() {
    ...
    case "$skill_name" in
        architecture-review|archify|ask-chatgpt-pro|ask-oracle|babysit-pr|bro|codex-project-init|design-experiment|diagnose|distill-source|doc-audit|docker-optimize|explain|fix-build|goal-prompt|grill-with-docs|gtsam-doc|integrate-research|logging-optimize|prepare-dynamic-workflow|prototype|research-prompt|review-change|review-experiment|session-handoff|skill-lifecycle|storage-cleanup|surface-unknowns|systemic-diagnosis|thermo-nuclear-code-quality-review|tdd|zoom-out) return 0 ;;
        *) return 1 ;;
    esac
}
