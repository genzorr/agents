...
is_repo_managed_skill() {
    case "$1" in
        architecture-review|archify|ask-chatgpt-pro|ask-oracle|babysit-pr|design-experiment|diagnose|distill-source|doc-audit|docker-optimize|fix-build|goal-prompt|grill-with-docs|gtsam-doc|integrate-research|logging-optimize|prepare-dynamic-workflow|prototype|research-prompt|review-change|review-experiment|session-handoff|skill-lifecycle|systemic-diagnosis|thermo-nuclear-code-quality-review|tdd|zoom-out) return 0 ;;
        *) return 1 ;;
    esac
}
